...
用户决定（2026-09-07）：按 Python 课程的既定模板，为每一节生成站点文档（**作业部分移除**）与互动教学课程。确认方案：课程页编号用 `db-0001~db-0007`（= 官方节号，避开 Python 的 0003–0032）；SQL 验证用 Docker postgres scratch 库实际执行；模板全量执行（分级测验 9 题 + 作业改编最终考核 + 面试实战 + 速查表）；面试实战按《面试实战模板 v2》（`INTERVIEW-TEMPLATE.md`，2026-09-09 定稿）执行。
...
- 每节课程：本课目标 → 知识讲解 → 分级测验（🌱/🌿/🎯 各 3 道）→ 最终考核（源作业改编、demo 内嵌；03/06/07 无作业 → 等效考核，先例 Python 26 章）→ 💼 面试实战（按 `INTERVIEW-TEMPLATE.md` v2：5 类题型、弹框单题推进、弹性题数）→ 巩固延伸
...
- 外部知识资源只用官方文档（PostgreSQL 官方文档等）；面试题来源遵循 `INTERVIEW-TEMPLATE.md` v2 题源协议（传统考点可场景化改写并注明；AI 原生题按来源层级标注）
