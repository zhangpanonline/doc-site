...
''',
    ...
    type: 'mechanism',
    level: '中级岗常问',
    prompt: `官方文档说 TRUNCATE 比 DELETE 快的机制原因是什么？磁盘空间回收上有什么区别？`,
    source: '来源：PostgreSQL 官方文档 SQL 命令参考「TRUNCATE」条目 · 检索 2026-09-09 · 层级：一手',
    breakdown: `原文两句话：① <strong>「it does not actually scan the tables」</strong>——不做逐行扫描（DELETE 逐行生成死元组），所以快；② <strong>「reclaims disk space immediately, rather than requiring a subsequent VACUUM operation」</strong>——磁盘空间<strong>立即回收</strong>，而 DELETE 后要等 VACUUM。语义上等价于<strong>无 WHERE 的 DELETE</strong>（原文：same effect as an unqualified DELETE）。`,
  },
  {
    type: 'trap',
    level: '中级岗常问',
    prompt: `AI 想「清理 90 天前的日志」写了 <code>TRUNCATE logs</code>。审查：按官方定义，这个命令实际做了什么？问题在哪？`,
    source: '来源：PostgreSQL 官方文档 SQL 命令参考「TRUNCATE」条目 · 检索 2026-09-09 · 层级：一手（场景化改写）',
    breakdown: `TRUNCATE 等价于<strong>无 WHERE 的 DELETE</strong>——不带任何过滤条件，<strong>整表瞬间清空</strong>，90 天前的没删、昨天的全没了，而且空间立即回收意味着<strong>更难恢复</strong>。正确姿势：按时间过滤的 DELETE 分批删（配 VACUUM），或用<strong>分区表</strong>按月滚删（DROP PARTITION）。审查 AI 的清理脚本：见到 TRUNCATE 先确认「整表清空」确实是意图。`,
  },
''',
    'db-0003-表间关系': r'''  {
    type: 'mechanism',
    level: '高级岗常问',
    prompt: `官方文档说外键对被引用的列有什么硬性要求？这个要求带来什么连锁效应？`,
    source: '来源：PostgreSQL 官方文档「Foreign Keys」小节（ddl-constraints）· 检索 2026-09-09 · 层级：一手',
    breakdown: `原文：外键必须引用<strong>主键、唯一约束列、或非部分唯一索引列</strong>——因此被引用列<strong>总是有索引</strong>（高效判断引用行是否存在）。连锁效应：父表 <strong>DELETE 行 / UPDATE 被引用列</strong>时，需要<strong>扫描引用表</strong>找匹配行——引用表的外键列若没索引，删一条父行就会<strong>全表扫子表</strong>，大表下删除变成灾难。`,
  },
  {
    type: 'trap',
    level: '高级岗常问',
    prompt: `AI 建的 order_items 表有外键列 order_id，但忘了给它建索引。审查：哪些操作会变慢？官方文档的哪句话解释了原因？`,
    source: '来源：PostgreSQL 官方文档「Foreign Keys」小节（ddl-constraints）· 检索 2026-09-09 · 层级：一手（场景化改写）',
    breakdown: `官方原文：「a DELETE of a row from the referenced table or an UPDATE of a referenced column will require <strong>a scan of the referencing table</strong>」——删订单、改订单 id 时都要在 order_items 里找匹配行；外键列没索引 → 每次都是<strong>顺序全表扫</strong>。修复：<code>CREATE INDEX ON order_items(order_id)</code>——外键列建索引是 PG 建表清单里的标准项，AI 生成 schema 十有八九漏。`,
  },
''',
    'db-0005-事务': r'''  {
    type: 'mechanism',
...
''',
    ...
    type: 'mechanism',
    level: '中级岗常问',
    prompt: `官方文档说视图是「物化」的吗？每次引用视图时数据库做什么？`,
    source: '来源：PostgreSQL 官方文档 SQL 命令参考「CREATE VIEW」条目 · 检索 2026-09-09 · 层级：一手',
    breakdown: `原文：<strong>「The view is not physically materialized. Instead, the query is run every time the view is referenced in a query.」</strong>——视图只是<strong>保存的查询</strong>，每次引用都重新执行底层 SQL。推论：视图不是缓存、不是性能优化手段——视图套视图时查询会层层展开，复杂视图的代价是实打实的。要「物化」用 MATERIALIZED VIEW（另需 REFRESH）。`,
  },
  {
    type: 'trap',
    level: '高级岗常问',
    prompt: `AI 想给视图加一列，用了 <code>CREATE OR REPLACE VIEW</code>。官方文档说这能成功吗？约束是什么？`,
    source: '来源：PostgreSQL 官方文档 SQL 命令参考「CREATE VIEW」条目 · 检索 2026-09-09 · 层级：一手（场景化改写）',
    breakdown: `不能随便改：原文要求新查询<strong>「must generate the same columns」——列名、列顺序、数据类型必须与旧视图完全一致</strong>，否则报错。改结构正确姿势：<code>DROP VIEW ... ; CREATE VIEW ...</code>（注意级联依赖，下游视图/函数会失效）。AI 生成 schema 迁移脚本时拿 OR REPLACE 当「万能更新」，是视图结构变更的头号坑。`,
  },
''',
    'db-0004-DML': r'''  {
    type: 'mechanism',
    level: '中级岗常问',
    prompt: `官方教程用 weather 与 cities 两表演示 LEFT OUTER JOIN——这个例子说明了什么语义？`,
    source: '来源：PostgreSQL 官方教程《3.5. Joins Between Tables》「LEFT OUTER JOIN」小节 · 检索 2026-09-09 · 层级：一手',
    breakdown: `<code>weather LEFT OUTER JOIN cities ON ...</code>：<strong>左表所有行全部保留</strong>——例子里 Hayward 一行在 cities 里没有匹配，城市字段全部为空照样出现（San Francisco 两行正常匹配）。教程原话：之所以叫 left，因为<strong>「the table mentioned on the left」</strong>的行全部输出。这是「保留未匹配行」语义的官方出处。`,
  },
  {
    type: 'trap',
    level: '高级岗常问',
    prompt: `基于官方教程的 LEFT JOIN 语义：在 WHERE 里过滤右表字段，为什么未匹配的行会消失（LEFT JOIN 变成内连接）？`,
    source: '来源：PostgreSQL 官方教程《3.5. Joins Between Tables》· 检索 2026-09-09 · 层级：一手（场景化改写，结论 pg16 实测）',
    breakdown: `LEFT JOIN 未匹配行的右表列是 <strong>NULL</strong>（官方例子里 Hayward 的城市字段为空）；WHERE 里任何对右表列的比较（如 <code>cities.population > 100</code>）对 NULL 判<strong>unknown → 行被滤掉</strong>。修复：过滤条件写进 <strong>ON</strong>（连接时过滤、保留左表行），或 WHERE 加 <code>OR 右表主键 IS NULL</code>。这条是 LEFT JOIN 相关面试题的第一高频坑。`,
  },
''',
    'db-0001-PostgreSQL安装': r'''  {
    type: 'mechanism',
    level: '中级岗常问',
    prompt: `Docker 官方说容器数据持久化的首选机制是什么？与 bind mount 相比，核心理由是什么？`,
    source: '来源：Docker 官方文档《Storage》「Volumes」节 · 检索 2026-09-09 · 层级：一手',
    breakdown: `官方原文：<strong>「Volumes are the preferred mechanism for persisting data」</strong>；理由对比：<strong>bind mount 依赖宿主机的目录结构与操作系统</strong>，而 <strong>volume 完全由 Docker 管理</strong>——官方还明确列出优势：<strong>「easier to back up or migrate」</strong>（备份/迁移更省心）、可用 Docker CLI/API 管理、多容器共享、支持远程存储驱动。`,
  },
  {
    type: 'design',
    level: '中级岗常问',
    prompt: `生产 PostgreSQL 数据目录用 volume；开发时想直接看数据文件、改配置用 bind mount。按 Docker 官方文档，这个分工的依据是什么？`,
    source: '来源：Docker 官方文档《Storage》「Volumes」节 · 检索 2026-09-09 · 层级：一手（场景化改写）',
    breakdown: `官方依据：volume <strong>与宿主机目录结构/OS 解耦</strong>（跨机器迁移、备份、团队协作行为一致）——生产首选；bind mount 直接映射宿主机路径——<strong>文件位置可预期、可现场编辑</strong>，适合开发调试，但依赖宿主机环境（官方原文：dependent on the directory structure and OS of the host machine）。口诀：<strong>数据归 volume，开发看文件用 bind</strong>。`,
  },
''',
    '0030-项目管理工具': r'''  {
    type: 'mechanism',
    level: '中级岗常问',
    prompt: `PEP 621 定义了 pyproject.toml 里的什么？dependencies 字段是什么？为什么它是现代 Python 打包的地基？`,
    source: '来源：PEP 621「Storing project metadata in pyproject.toml」· 检索 2026-09-09 · 层级：一手',
    breakdown: `PEP 621 把<strong>项目元数据标准化进 pyproject.toml 的 [project] 表</strong>：name/version/<strong>dependencies</strong>（必需依赖列表）/optional-dependencies（可选依赖组）等。意义：元数据从此<strong>单一来源</strong>——pip/uv/poetry/hatchling 读同一份声明，告别 setup.py/setup.cfg/requirements.txt 各写一遍的散装时代；锁文件（uv.lock）则在它的基础上做版本解析。`,
  },
  {
    type: 'trap',
    level: '高级岗常问',
    prompt: `老项目同时有 setup.py、setup.cfg、requirements.txt，AI 建议「再补一个 pyproject.toml 就现代化了」。审查：多份依赖声明的真正风险是什么？`,
    source: '来源：PEP 621「Storing project metadata in pyproject.toml」· 检索 2026-09-09 · 层级：一手（场景化改写）',
    breakdown: `PEP 621 的核心动机就是<strong>消除元数据分散</strong>：多份声明并存时，<strong>哪个文件说了算</strong>随工具而变（pip 读 setup.py、uv 优先 pyproject、部署脚本可能还看 requirements.txt）——改依赖只改一处，另一处悄悄过期，装出来的环境跟预期不同。正确迁移：以 pyproject [project] 为唯一真源，删掉其余声明（保留 requirements 只用于特殊锁定场景），用 uv sync 重建锁文件。`,
  },
''',
    '0031-monorepo': r'''  {
    type: 'mechanism',
    level: '中级岗常问',
    prompt: `uv 官方文档给 workspace 的定义原文是什么？灵感来自哪里？`,
    source: '来源：uv 官方文档《Workspaces》概念页 · 检索 2026-09-09 · 层级：一手',
    breakdown: `原文：<strong>「a workspace is a collection of one or more packages, called workspace members, that are managed together」</strong>——灵感来自 Cargo 的同名概念。动机原文：<strong>把大型代码库拆成多个包、共享依赖一起管理</strong>——官方举例：FastAPI Web 应用 + 它旁边的一组库。成员包各自独立、又在同一依赖锁与工具链下协同。`,
  },
  {
    type: 'design',
    level: '中级岗常问',
    prompt: `按 uv 官方文档的描述，什么样的项目适合组织成 workspace？判断标准是什么？`,
    source: '来源：uv 官方文档《Workspaces》概念页 · 检索 2026-09-09 · 层级：一手（场景化改写）',
    breakdown: `官方给出的适用画像：<strong>「organize large codebases by splitting them into multiple packages with common dependencies」</strong>——应用 + 多个内部库、共享同一批依赖、需要一起改一起发。判断标准：包之间<strong>是否频繁一起改</strong>（改 API 同步改调用方、统一锁文件保证版本一致）。完全独立、各自团队的包放多仓库更省心。`,
  },
''',
}
