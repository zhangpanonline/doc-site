...
- **选项位置必须打乱（2026-09-09 修复后强制）**：批量生成时代曾有 26 节 Python 课答案 100% 在 A 位（学员点 A 可过全部测验）。新课程发布前跑 `teach/tools/shuffle_options.py`（确定性洗牌：以文件名+题序号为种子，正确选项随内容移动、data-correct 同步更新），并用「新旧正确选项内容零漂移 + 分布近均匀（各选项 20%~30%）」两个标准验收。脚本幂等，重复跑结果一致。
