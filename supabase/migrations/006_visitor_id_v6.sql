-- 统计 v6：访客去重（代理换 IP 不再虚增独立用户）
-- 用法：Supabase 控制台 → SQL Editor → 整体执行（幂等）。
--
-- 本文件相对以往多一步 DDL：给 visits 加 visitor_id 列（匿名 cookie，服务端签发）。
-- 老数据该列为 NULL，统计时用 coalesce(visitor_id, 'ip:'||ip) 兼容（旧行按 IP 计一个访客）。
...
as $$
  ...
  -- 访客级地区归属：每个访客（cookie 或 IP 兜底）取其最近一次访问的地区
  visitor_region as (
  ...
  province_stats as (
    ...
    left join (
      select coalesce(region, '未知') as region, count(*)::int as visitors
      from visitor_region
      group by 1
    ) vr using (region)
  ),
  ips as (
    select ip,
           count(*)::int as c,
           count(distinct coalesce(visitor_id, 'ip:' || ip))::int as visitors,
  ...
  select jsonb_build_object(
    ...
    'province_stats', (select coalesce(jsonb_agg(jsonb_build_object('region', region, 'count', c, 'ips', ips, 'visitors', coalesce(visitors, 0)) order by c desc), '[]'::jsonb) from province_stats),
    'ips',        (select coalesce(jsonb_agg(jsonb_build_object('ip', ip, 'count', c, 'visitors', visitors, 'first', first_seen, 'last', last_seen, 'last7', last7, 'country', country, 'region', region, 'city', city, 'user_agent', user_agent) order by c desc), '[]'::jsonb) from ips),
